...
RESULTS_DIR = 'benchmark_debug_case_58'
...
# ==========================================
# 评测主流程 (精准硬编码 Waypoints)
# ==========================================
@single_experiment_yaml
def run_hardcoded_debug_case(
    ...
    **kwargs
    ...
    print(f'🚀 Initializing Debug Mode with Hardcoded Waypoints...')
    ...
    # 1. 精确复原 Trial 的环境 (使用原来的 seed=42)
    dynamic_env = EnvDense2DExtraObjects(tensor_args=tensor_args, drop_old_num=2, num_extra_spheres=12, num_extra_boxes=12, seed=seed)
    ...
    waypoints_np = np.array(hardcoded_wps)
    waypoints_t = [torch.tensor(wp, **tensor_args) for wp in hardcoded_wps]
    num_segments = len(waypoints_np) - 1

    print(f"\n================ [ DEBUGGING HARDCODED CASE ] ================")
    ...
    for seg in range(num_segments):
        ...
        print(f"\n--- Segment {seg+1} (from {start_np} to {goal_np}) ---")

        for method in methods_list:
            state = tracker[method]
            if state["fatal_error"]: 
                print("Skipping due to fatal error in previous segment.")
                continue
            ...
            if method == "TMPD (Ours)":
                attempts = 0
                max_tmpd_attempts = 50 
                current_noise_std = 1.8 
                ...
                while best_traj_np is None and attempts < max_tmpd_attempts:
                    ...
                    if attempts > 10:
                        ...
                        print(f"    [TMPD] Attempt {attempts}: Increasing noise to {current_noise_std:.1f}")

                    h_cond = dataset.get_hard_conditions(torch.vstack((start_t, goal_t)), normalize=True)
                    t_samples = model.run_inference(None, h_cond, n_samples=n_samples, horizon=n_support_points, return_chain=True, sample_fn=ddpm_sample_fn, guide=guide, n_guide_steps=10, t_start_guide=ceil(start_guide_steps_fraction * model.n_diffusion_steps), noise_std_extra_schedule_fn=lambda x: current_noise_std, n_diffusion_steps_without_noise=n_diffusion_steps_without_noise)
                    ...
                    if free_t is not None:
                        ...
                        if best_traj_np is None and len(unique_mpd_classes) > 0:
                            ...
                            if attempts % 5 == 0: print(f"    [TMPD] Attempt {attempts}: Candidates generated, but ALL tangled.")
                    else:
                        state["failed_traj"] = t_unnorm[0, ..., :2].cpu().numpy()
                        if attempts % 5 == 0: print(f"    [TMPD] Attempt {attempts}: ALL candidates collided with obstacles.")
    ...
    fig.suptitle(f"Debug Mode - Hardcoded Waypoints", fontsize=18, weight='bold')

    st = tracker["TMPD (Ours)"]
    dynamic_env.render(ax)
    ax.set_title("TMPD (Ours)", fontsize=16, weight='bold', color='darkred')
    ...
    if st["failed_traj"] is not None:
        ...
        ax.plot(f_traj[:, 0], f_traj[:, 1], color='red', linewidth=2.5, linestyle=':', alpha=0.8, zorder=18, label='Failed Attempt Path')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f"debug_hardcoded_waypoints.png"), dpi=200); plt.close(fig)
    print(f"\n✅ Debug plot saved to '{RESULTS_DIR}/debug_hardcoded_waypoints.png'")

if __name__ == '__main__':
    run_experiment(run_hardcoded_debug_case)