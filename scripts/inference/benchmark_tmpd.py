...
RESULTS_DIR = 'benchmark_all_fast_results'
...
# 算法核心函数
# ==========================================
def lifelong_topo_rrt(start, goal, env, initial_W, step_size=0.05, W_th=0.95, robot_radius=0.03, max_iters=50000):
...
def lifelong_topo_a_star(start, goal, env, initial_W, step_size=0.05, W_th=0.95, robot_radius=0.03):
    ...
    iters = 0
    while open_set and iters < 50000:
        iters += 1
...
def run_fast_benchmark(
    ...
    num_segments: int = 5,
    n_samples: int = 100,
    n_diffusion_steps_without_noise: int = 20, 
    start_guide_steps_fraction: float = 0.20,
    device: str = 'cuda',
    seed: int = 42,
    ...
    **kwargs
    ...
    print(f'🚀 Initializing All-in-One Fast Benchmark...')
    ...
    for trial in range(num_trials):
        ...
        tracker = {m: {"history": [], "sr": 0, "time": 0.0, "tangled": False, "collision": False, "pl_list": [], "sm_list": []} for m in methods_list}
        ...
        for seg in range(num_segments):
            ...
            for method in methods_list:
                ...
                if state["collision"] or state["tangled"]: continue
                t0 = time.time()
                best_traj_np = None
                ...
                if method == "Topo-RRT":
                    path_list = lifelong_topo_rrt(start_np, goal_np, dynamic_env, initial_W, step_size=0.05, robot_radius=0.03)
                    ...
                    t_samples = model.run_inference(None, h_cond, n_samples=n_samples, horizon=n_support_points, return_chain=True, sample_fn=ddpm_sample_fn, guide=guide, n_guide_steps=10, t_start_guide=ceil(start_guide_steps_fraction * model.n_diffusion_steps), noise_std_extra_schedule_fn=lambda x: 0.5, n_diffusion_steps_without_noise=5)
                    _, _, free_t, _, _ = dynamic_task.get_trajs_collision_and_free(dataset.unnormalize_trajectories(t_samples)[-1], return_indices=True)
                    if free_t is not None: best_traj_np = free_t[0, ..., :2].cpu().numpy()

                elif method == "TMPD (Ours)":
                    h_cond = dataset.get_hard_conditions(torch.vstack((start_t, goal_t)), normalize=True)
                    t_samples = model.run_inference(None, h_cond, n_samples=n_samples, horizon=n_support_points, return_chain=True, sample_fn=ddpm_sample_fn, guide=guide, n_guide_steps=10, t_start_guide=ceil(start_guide_steps_fraction * model.n_diffusion_steps), noise_std_extra_schedule_fn=lambda x: 1.8, n_diffusion_steps_without_noise=n_diffusion_steps_without_noise)
                    _, _, free_t, _, _ = dynamic_task.get_trajs_collision_and_free(dataset.unnormalize_trajectories(t_samples)[-1], return_indices=True)
                    ...
                    if free_t is not None:
                        ...
                        if best_traj_np is None:
                            state["tangled"] = True
                            best_traj_np = unique_mpd_classes[np.argsort(preliminary_scores)[0]]
        ...
        fig.suptitle(f"Continuous Lifelong Navigation Fast Benchmark - Trial {trial+1}", fontsize=22, weight='bold')
        axes = axes.flatten()
        ...
        for i, method in enumerate(methods_list):
            ...
            for seg_idx, traj in enumerate(st["history"]):
                ax.plot(traj[:, 0], traj[:, 1], color=line_color, linewidth=2.0, alpha=0.6, label='Physical Path' if seg_idx==0 else "")
                mid = len(traj) // 2
                ...
                if taut_traj is not None:
                    ...
                    if st["tangled"]:
                        ...
                        for obs_idx, w in enumerate(sig):
                            if abs(w) >= 0.95: ax.plot(obs_centers_np[obs_idx][0], obs_centers_np[obs_idx][1], 'rx', markersize=18, markeredgewidth=3, zorder=25)
            
            handles, labels = ax.get_legend_handles_labels()
            if handles: ax.legend(loc='lower right', fontsize=11, framealpha=0.9)                    
            
            if st["collision"]: status_text, box_color = "Collision (Failed)", 'lightgray'
            elif st["tangled"]: status_text, box_color = "Tangled (Failed)", 'lightcoral'
            ...
            all_results.append({
                ...
                "Tangle_Free_Rate": 1.0 if (not st["tangled"] and st["sr"] == num_segments) else 0.0,
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(RESULTS_DIR, f"fast_benchmark_trial_{trial+1}.png"), dpi=200); plt.close(fig)
    ...
    summary = df.groupby("Method").agg({
        ...
        "Avg_Seg_Time": lambda x: f"{np.nanmean(x):.2f}s",
...
if __name__ == '__main__':
    run_experiment(run_fast_benchmark)