...
from mpd.utils.bench_io import ensure_dir, resolve_output_root, save_dataframe_csv
from mpd.utils.bench_metrics import format_single_pairs_summary
from mpd.utils.loading import load_params_from_yaml
from mpd.utils.waypoints import sample_collision_free_start_goal
...
PLOTS_DIR = os.path.join(RESULTS_ROOT, "single_pairs")
METRICS_CSV_PATH = os.path.join(RESULTS_ROOT, "single_pairs", "final_random_overlay_benchmark_metrics.csv")
...
def run_benchmark_and_plot(
    ...
    **kwargs
    ...
    output_dir = resolve_output_root(PLOTS_DIR, results_dir)
    metrics_csv_path = os.path.join(output_dir, os.path.basename(METRICS_CSV_PATH))
    ...
    for trial_id in range(num_trials):
        ...
        # Sample collision-free start/goal pair.
        ...
        if trajs_free_v is not None:
            ...
            # Compute topology energy for the selected MPD trajectory.
        ...
        # Render and save overlay figure.
        ...
        plt.tight_layout()
        plot_path = os.path.join(output_dir, f"overlay_{trial_id+1:03d}.png")
        plt.savefig(plot_path, dpi=200)
        plt.close(fig) 
        
    # Final summary table.
    df = pd.DataFrame(results)
    save_dataframe_csv(df, metrics_csv_path, index=False)
    
    print("\n" + "="*90)
    print("SINGLE-TASK ABLATION BENCHMARK (RANDOM POINTS OVERLAY)")
    print("="*90)
    summary = format_single_pairs_summary(df)
    print(summary.to_string())
    print("="*90)
    print(f"{num_trials} overlay figures saved to '{output_dir}/'!")
