...
def collect_polyline_from_clicks(fig, ax, env, title: str, min_points: int = 2):
    """
    Collect a polyline from mouse clicks.
    Left click: add point. Right click or Enter: finish.
    ...
    def redraw():
        ...
        if len(points) > 0:
            ...
            ax.plot(pts[:, 0], pts[:, 1], color='black', linewidth=2.2, alpha=0.9)
...
def save_denoise_animation_multi(
    ...
    if num_steps >= n_steps:
        frame_indices = list(range(n_steps))
    else:
        frame_indices = np.linspace(0, n_steps - 1, num=num_steps, dtype=int).tolist()
...
def experiment(
    ...
    save_denoise_animation_flag: bool = True,
    animation_fps: int = 10,
    animation_num_steps: int = 30,
    animation_format: str = 'gif',
    max_animation_candidates: int = 0,
    fixed_history: str = '',
    interactive_history: bool = True,
    **kwargs
):
    ...
    if interactive_history and not render:
        raise ValueError("interactive_history=True requires render=True.")
    ...
    if render:
        ...
        if interactive_history and fixed_history_np is None:
            print("Please draw a fixed history path: left click add points, right click or Enter to finish.")
            clicked_history = collect_polyline_from_clicks(
                fig_topo,
                ax_topo,
                env,
                title="Draw fixed history: Left click add | Right click/Enter finish",
                min_points=2,
            )
            if clicked_history is None:
                print("No valid history drawn, keep default empty history.")
            else:
                history_path_list = [clicked_history.copy()]
                start_state_pos[:2] = torch.tensor(clicked_history[-1], **tensor_args)
                current_start_pos = start_state_pos.clone()
                print(f"Interactive history loaded with {len(clicked_history)} points.")
        ...
        if trajs_final_free is not None:
            ...
            if save_denoise_animation_flag and trajs_free_idxs is not None and trajs_free_idxs.nelement() > 0:
                ...
                if len(free_batch_indices) == len(trajs_free_np):
                    if max_animation_candidates <= 0:
                        max_candidates = len(trajs_free_np)
                    else:
                        max_candidates = max(1, int(max_animation_candidates))

                    # Match selected trajectory to one sample in All MPD for denoising chain lookup.
                    best_dist = np.linalg.norm(trajs_free_np - latest_best_traj_raw[None, ...], axis=(1, 2))
                    best_local_idx = int(np.argmin(best_dist))

                    ordered_all_idxs = [best_local_idx] + [
                        i for i in range(len(trajs_free_np)) if i != best_local_idx
                    ]
                    selected_all_idxs = ordered_all_idxs[:max_candidates]
                    ...
                    for i, source_idx in enumerate(selected_all_idxs):
                        ...
                        if source_idx == best_local_idx:
                            candidate_finals.append(np.asarray(latest_best_traj))
                            candidate_labels.append("All MPD Best")
                            best_candidate_idx = len(candidate_chains) - 1
                        else:
                            candidate_finals.append(np.asarray(trajs_free_np[source_idx]))
                            candidate_labels.append(f"All MPD #{source_idx}")
